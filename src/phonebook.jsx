    ...
    return(
        <>
            {list.map((v)=>{
               ...
               <> 
               <p>{v.name} {v.number}</p>
               <button onClick={()=>{func(v)}}>Delete</button>
               </>) 
            })}
        </>
    )
...
const AppPhoneBook=()=>{
    ...
    useEffect(()=>{
        PhoneBookService.getPhoneBook().then((res)=>{
            newPerson(res.data)
        }).catch(()=>{
    ...
    return(
        ...
        <span>Filter by name :</span><input onChange={(e)=>{
        ...
        <form method="post" onSubmit={(e)=>{
            ...
            if(person.find((v)=>{
                ...
                if(confirm(`data with the name ${personInput} already Exist Do you want to Replace The Current Data?`)){
                    PhoneBookService.updatePhoneBook((person.filter((v)=>{
                        return v.name==personInput
                    }))[0],personObject).then((res)=>{
                ...
                PhoneBookService.postPhoneBook(personObject).then((res)=>{
                    console.log('Posted')
        ...
        <span>Name   :</span><input onChange={(e)=>{inputOnchange(e,newPersonInput)}} /><br/>
        <span>Number :</span><input onChange={(e)=>{inputOnchange(e,newNumberInput)}} /><br/>
